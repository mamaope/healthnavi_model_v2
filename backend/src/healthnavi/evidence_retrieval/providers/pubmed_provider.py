...
from healthnavi.evidence_retrieval.providers.base import (
...
class PubMedProvider(BaseEvidenceProvider):
    ...
    def _get_xml(self, client: httpx.Client, pmids: list[str]) -> str:
        ...
        for attempt in range(3):
            try:
                response = client.get(self.EFETCH_URL, params=clean_params)
                response.raise_for_status()
        ...
        raise ProviderError("PubMed EFetch failed")
