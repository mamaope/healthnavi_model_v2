...
from healthnavi.evidence_retrieval.models import EvidenceItem, SearchResult
from healthnavi.evidence_retrieval.providers.base import BaseEvidenceProvider, ProviderError
from healthnavi.evidence_retrieval.providers.crawl4ai_provider import Crawl4AIProvider
from healthnavi.evidence_retrieval.providers.europe_pmc_provider import EuropePMCProvider
from healthnavi.evidence_retrieval.providers.official_health_api_provider import (
    OfficialHealthAPIProvider,
)
from healthnavi.evidence_retrieval.providers.pubmed_provider import PubMedProvider
from healthnavi.evidence_retrieval.providers.semantic_scholar_provider import (
...
class EvidenceSearchService:
    ...
    def search(self, question: str, top_k: int = 8) -> SearchResult:
        ...
        normalized_query = query_plan.ranking_query
        ranking_query = query_plan.ranking_query or normalize_question(question) or question.strip()
        ...
        try:
            futures = {
                executor.submit(
                    ...
                    provider,
                    build_provider_query(query_plan.original, provider.source_name),
                    max_results,
                ): provider
    ...
    def _default_providers(self) -> list[BaseEvidenceProvider]:
        ...
        crawl_only = self.provider_mode == "crawl"
        if not crawl_only and self.settings.enable_pubmed:
            providers.append(PubMedProvider(self.settings))
        if not crawl_only and self.settings.enable_europe_pmc:
            providers.append(EuropePMCProvider(self.settings))
        if not crawl_only and self.settings.enable_semantic_scholar:
...
def _normalize_provider_mode(value: str | None) -> str:
    ...
    if normalized in {"crawl", "crawled", "crawler", "web_crawl", "crawl_only"}:
        return "crawl"
    return "web"
