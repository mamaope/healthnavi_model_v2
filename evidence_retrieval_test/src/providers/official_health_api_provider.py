...
class OfficialHealthAPIProvider(BaseEvidenceProvider):
    ...
    def _search_who_gho(self, query: str, max_results: int) -> list[EvidenceItem]:
        if not _looks_like_population_data_query(query):
            return []
        ...
        with httpx.Client(timeout=self.timeout) as client:
            ...
            for indicator in indicators[:max_results]:
                ...
                if not code or not name:
                    continue
                country_rows = self._fetch_who_country_rows(client, code)
                items.append(_who_indicator_to_item(code, name, country_rows, query))
        return items
    ...
    def _fetch_who_country_rows(self, client: httpx.Client, indicator_code: str) -> list[dict]:
        data = self._get_json(
            client,
            f"{self.WHO_API_BASE}/{indicator_code}",
            {
                "$filter": " or ".join(
                    f"SpatialDim eq '{country}'" for country in self.COUNTRY_CODES
                ),
    ...
    def _search_world_bank_country_health(
        ...
        terms = _search_terms(query)
        if not terms or not _looks_like_population_data_query(query):
            return []
        ...
        with httpx.Client(timeout=self.timeout) as client:
            for indicator in matched:
                url = (
                    f"{self.WORLD_BANK_API_BASE}/country/UGA;ZMB/indicator/"
                ...
                items.append(_world_bank_indicator_to_item(indicator, rows, query, url))
        return items
...
def _who_indicator_to_item(
    ...
    api_url = _url_with_params(
        ...
        {
            "$filter": "SpatialDim eq 'UGA' or SpatialDim eq 'ZMB'",
            "$orderby": "TimeDimensionBegin desc",
        },
    )
    return EvidenceItem(
...
def _world_bank_indicator_to_item(
    ...
    return EvidenceItem(
        ...
        snippet=snippet
        or "World Bank health/development indicator for Uganda and Zambia.",
...
def _search_terms(query: str) -> list[str]:
    return [
        ...
        if term not in _LOCATION_AND_CONTEXT_TERMS
    ]
...
def _looks_like_population_data_query(query: str) -> bool:
    q = query.lower()
    return any(term in q for term in _POPULATION_DATA_TERMS)
...
def _query_tokens(query: str) -> list[str]:
    ...
    for token in re.sub(r"[^a-zA-Z0-9\s]", " ", query.lower()).split():
        if token.isdigit() or len(token) < 3 or token in _NON_CONTENT_TERMS or token in seen:
            continue
        seen.add(token)
        tokens.append(token)
    return tokens
...
def _country_label(code: str) -> str:
    return {"UGA": "Uganda", "ZMB": "Zambia"}.get(code, code)
